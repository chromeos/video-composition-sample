...
class SelectedClipAdapter(dataList: MutableList<TrackModel> = ArrayList(),
                          var onRemoveClickListener: ((TrackModel) -> Unit)? = null,
                          var onEffectClickListener: ((TrackModel) -> Unit)? = null,
                          callback: ((TrackModel) -> Unit)? = null)
    : BaseRecyclerViewAdapter<TrackModel>(dataList, R.layout.selected_clip_item, callback) {
    ...
    inner class Holder(override val containerView: View)
        ...
        fun bind(data: TrackModel, position: Int) {
            tvClipTitle.text = containerView.context.getString(data.nameResId)
            tvPosition.text = (position + 1).toString()

            tvEffect.html = containerView.context.getString(R.string.settings__effect, data.effectId)
            tvEffect.setOnClickListener { onEffectClickListener?.invoke(data) }
            ivRemove.setOnClickListener { onRemoveClickListener?.invoke(data) }
        }
    }
